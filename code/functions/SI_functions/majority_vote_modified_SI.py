...
def majority_vote_modified_SI(num_unlabeled, membership_y,feature_x, num_iter, cv_setup=None, threshold_cutoff = None):
    ...
    for i in range(len(num_unlabeled)):
        ...
        for k, (train, test) in enumerate(k_fold):
            ...
            if threshold_cutoff == 'manual':
                accuracy_score_benchmark = threshold_cutoff
            else:
            ...
            if len(np.unique(membership_y))>2: ## k>2 classes
                ...
                accuracy.append(f1_score(y_true, y_pred, average='macro'))#, pos_label=1) )
                    #print fp+tn
                    #print tp+fn
                    #accuracy0.append(tn/(fp+tn))#f1_score(y_true, y_pred, average='binary', pos_label=0) )
                    #accuracy1.append(tp/(tp+fn))#f1_score(y_true, y_pred, average='binary', pos_label=1) )
                accuracy0.append(f1_score(y_true, y_pred, average='binary', pos_label=0) )
                accuracy1.append(f1_score(y_true, y_pred, average='binary', pos_label=1) )
        mean_accuracy.append(np.mean(accuracy)) #placeholder
        se_accuracy.append(np.std(accuracy)) #placeholder
        mean_accuracy0.append(np.mean(accuracy0)) #placeholder
        se_accuracy0.append(np.std(accuracy0)) #placeholder
        mean_accuracy1.append(np.mean(accuracy1)) #placeholder
        se_accuracy1.append(np.std(accuracy1)) #placeholder
    ...
    return(mean_accuracy, se_accuracy, mean_micro_auc,se_micro_auc, mean_wt_auc,se_wt_auc,mean_accuracy0,se_accuracy0,mean_accuracy1,se_accuracy1)
